...
public class Gestion_Assistants3_Controller extends MenuAdministrateur{
	@FXML
	...
	public void add(ActionEvent event) throws IOException {
		...
		if (newName.getText().isBlank() || newLogin.getText().isBlank() || newPass.getText().isBlank() || newPass2.getText().isBlank() || newCIN.getText().isBlank() || newTel.getText().isBlank() || adminPass.getText().isBlank()) {
			...
			addMsg.setText("La CIN existe d�j�");
		}
		else if (existNom(newName.getText())) {
			addMsg.setText("Le nom existe d�j�");
		}
		else if (existLogin(newLogin.getText())) {
			addMsg.setText("Le login existe d�j�");
		}
		else {
	    	...
	    	confirm.setContentText("le nouvel utilisateur a �t� cr�� avec succ�s");
	...
	public void delete(ActionEvent event) throws IOException {
		...
		if (assistantSelected != null) {			
				...
				if (!popupwindow.isShowing()) {
					if (g.passValide()) {
						...
						try {
					    	...
					    	confirm.setHeaderText(null);
					    	confirm.setTitle("Confirmation");
					    	confirm.setContentText("Cet assistant a �t� supprim� avec succ�s");
					    	Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
					    	confirm.initOwner(stage);
					    	confirm.show();
			...
			alert.setContentText("s�lectionner l'assistant � supprimer");
			Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
	...
	public void edit(ActionEvent event) throws IOException {
		...
		if (assistantSel != null) {			
			...
			estAssist = true;
			AuthentificationController.src = "EditUsers.fxml";
			Stage popupwindow=new Stage();	
			popupwindow.setResizable(false);
			popupwindow.initModality(Modality.APPLICATION_MODAL);
			popupwindow.setTitle("Authentification");
