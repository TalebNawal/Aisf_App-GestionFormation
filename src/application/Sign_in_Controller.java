...
public class Sign_in_Controller {
	...
	@FXML
	private ToggleButton button_admin;
	@FXML
	...
	public boolean isAdmin() {
		if (isD) {
			return ((Administrateur) utilisateur).isAdmin();
		}
		return false;
	}
	...
	public void SeConnecter(ActionEvent event) throws IOException {
		...
		if (login.getText().isBlank() || passwd.getText().isBlank()){
			...
			if (button_admin.isSelected()) {
				...
				File dentFile = new File("administrateurs"); 
				...
				// Verifier login & passwd pour le Dentiste
					...
					try{
						FileInputStream fis = new FileInputStream("administrateurs");
						while (fis.available() > 0) {
							ObjectInputStream ois = new ObjectInputStream(fis);
							utilisateur = (Administrateur) ois.readObject();
